require 'rails_helper'

describe 'Answers API' do
	let(:question) { create(:question) }
	let(:access_token) { create(:access_token) }
	let!(:answers)     { create_list(:answer, 2, question: question) }
	let!(:answer)      { answers.first }

	describe 'GET /index' do
		it_behaves_like "check API Authentication"

		context 'authorized' do

			before { get "/api/v1/questions/#{question.id}/answers", format: :json, access_token: access_token.token }

			it 'returns 200 status code' do
				expect(response).to be_success
			end

			it 'returns list of answers' do
				expect(response.body).to have_json_size(2).at_path('answers')
			end

			%w(id body created_at updated_at user_id best).each do |attr|
				it "answer object contains #{attr}" do
					expect(response.body).to be_json_eql(answer.send(attr.to_sym).to_json).at_path("answers/0/#{attr}")
				end
			end
		end

		def check_authentication(options = {})
			get "/api/v1/questions/#{question.id}/answers", { format: :json}.merge(options)
		end
	end

	describe 'GET /show' do
		let!(:comments) { create_list(:comment, 2, commentable: answer) }
		let!(:attachment) { create_list(:attachment,2, attachable: answer) }

		it_behaves_like "check API Authentication"

		context 'authorized' do
			context 'with valid question id' do

				before { get "/api/v1/answers/#{answer.id}", format: :json, access_token: access_token.token }

				it 'returns 200 status code' do
					expect(response).to be_success
				end

				%w(id body created_at updated_at user_id best).each do |attr|
					it "answer object contains #{attr}" do
						expect(response.body).to be_json_eql(answer.send(attr.to_sym).to_json).at_path("answer/#{attr}")
					end
				end

				context 'comments' do
					it 'included in answer object' do
						expect(response.body).to have_json_size(2).at_path("answer/comments")
					end

					%w{id body created_at updated_at}.each do |attr|
						it "contains #{attr}" do
							expect(response.body).to be_json_eql(comments.last.send(attr.to_sym).to_json).at_path("answer/comments/0/#{attr}")
						end
					end
				end

				context 'attachments' do
					it 'included in answer object' do
						expect(response.body).to have_json_size(2).at_path("answer/attachments")
					end

					it 'contains attachment url' do
						expect(response.body).to be_json_eql(attachment.last.file.url.to_json).at_path("answer/attachments/0/url")
					end
				end
			end
		end

		def check_authentication(options = {})
			get "/api/v1/answers/#{answer.id}", { format: :json }.merge(options)
		end
	end

	describe 'POST /create' do
		let(:create_answer) { post "/api/v1/questions/#{question.id}/answers",
															 access_token: access_token.token,
															 answer: attributes_for(:answer),
															 question: question,
															 format: :json }

		it_behaves_like "check API Authentication"

		context 'authorized' do
			it 'return status 201' do
				create_answer
				expect(response.status).to eq 201
			end

			it 'creates new answer' do
				expect{ create_answer }.to change(question.answers, :count).by(1)
			end
		end

		def check_authentication(options = {})
			post "/api/v1/questions/#{question.id}/answers", { answer: attributes_for(:answer), question_id: question, format: :json }.merge(options)
		end
	end
end